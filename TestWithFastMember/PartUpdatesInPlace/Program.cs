...
namespace PartUpdatesInPlace
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			File.Delete(ExtensionsPath + "Extensions2.dll");
			directoryCatalog.Refresh();
			...
			//File.Copy(BinDir + @"..\..\..\PartUpdatesInPlaceExtensions\bin\debug\PartUpdatesInPlaceExtensions.dll", ExtensionsPath + "Extensions1.dll");
			File.Copy(rebuildPluginPath,ExtensionsPath + "Extensions2.dll");
			directoryCatalog.Refresh();
		...
		private static string RebuildPluginWithRoslyn()
		{
			...
			var syntaxTree = SyntaxTree.ParseFile(@"..\..\..\PartUpdatesInPlaceExtensions2\Bar2.cs");
			
			var extensions1Dll = "Extensions2.dll";
			...
			var compilation = Compilation.Create(extensions1Dll,
				...
				.AddReferences(new MetadataFileReference(typeof(TypeAccessor).Assembly.Location));
